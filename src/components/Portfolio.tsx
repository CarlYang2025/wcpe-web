...
interface PortfolioLeg {
  ...
  market: string; match: string; group: string
}
...
interface AllocationItem {
  tier: string; pct: number; amount: number; isCash?: boolean
  portfolio?: { odds: number; hitPct: number; ev: number } | null
}
...
interface PortfolioData {
  ...
  investmentPool: { match: string; market: string; bet: string; odds: number; ev: number; mvi: number; reason: string }[]
  portfolios: PortfolioItem[]
  capitalAllocation: AllocationItem[]
  finalVerdict: { verdict: string; reason: string; bestPortfolio?: { score: number; odds: number; legs: PortfolioLeg[] } | null }
  yesterdayReview?: { date: string; summary: Record<string, number>; keyInsights: string[] }
  candidatePool?: { total: number; accepted: number; rejected: number }
  bestPortfolio?: { tier: string; score: number; odds: number; hitPct: number; ev: number; mvi: number; legs: PortfolioLeg[]; breakdown: Record<string, number>; rationale: string } | null
  stats?: { totalCombinations: number; topScore: number }
}
...
export default function Portfolio({ data }: { data: PortfolioData | null }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ③ Portfolio Search Results (Top 10) */}
      ...
      <div className="text-center text-[9px] text-[#555555] py-2">
        基于概率模型推演，仅供参考，不构成投注建议 | Portfolio Optimizer V{data.version}
      </div>
    </div>
  )
}
    ...
    <div className={`bg-[#1a1f3a] border-l-4 ${verdictBg} rounded-xl p-5`}>
      ...
      {(rating.highlights || []).filter(Boolean).length > 0 && (
        <div className="mt-2 space-y-1">
          {(rating.highlights || []).filter(Boolean).slice(0, 3).map((h, i) => (
  ...
  return (
    <div className="bg-[#1a1f3a] border border-[#141937] rounded-xl p-5">
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <MiniStat label="Top3比分" value={`${Math.round((s.top3Rate || 0) * 100)}%`} color="#54a0ff" />
        <MiniStat label="平局" value={`${s.draws || 0}/${s.n || 0}`} color="#a855f7" />
      </div>
      {expanded && (review.keyInsights || []).length > 0 && (
        <div className="space-y-1 mt-3 pt-3 border-t border-[#141937]">
...
function InvestmentPool({ pool, candidatePool }: { pool: PortfolioData['investmentPool']; candidatePool?: PortfolioData['candidatePool'] }) {
  ...
  return (
    <div className="bg-[#1a1f3a] border border-[#141937] rounded-xl p-5">
      <div className="flex items-center justify-between mb-3 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <h2 className="text-sm font-bold text-white">② 今日投资池 <span className="text-[10px] text-[#555555]">({pool?.length || 0}项)</span></h2>
        <span className="text-[10px] text-[#555555]">{expanded ? '收起 ▲' : `展开全部 ${pool?.length || 0} 项 ▼`}</span>
      </div>
      <div className="grid grid-cols-2 gap-1.5">
        {(pool || []).slice(0, expanded ? 100 : 6).map((item, i) => (
  ...
  return (
    <div className="bg-[#1a1f3a] border border-[#141937] rounded-xl p-5">
      ...
      <div className="space-y-2">
        {allocations.map((a, i) => (
          <div key={i} className={`flex items-center justify-between p-3 rounded-lg ${a.isCash ? 'bg-[#00ff88]/5 border border-[#00ff88]/20' : 'bg-[#141937]'}`}>
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-white w-16">{a.tier}</span>
              {a.portfolio && <span className="text-[10px] text-[#a0a0a0]">{a.portfolio.odds}x · {a.portfolio.hitPct}%命中</span>}
              {a.isCash && <span className="text-[10px] text-[#00ff88]">保留灵活资金</span>}
            </div>
            <div className="flex items-center gap-3">
              <div className="w-20 h-2 bg-[#0a0e27] rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${a.isCash ? 'bg-[#00ff88]' : 'bg-[#ffd700]'}`} style={{ width: `${a.pct}%` }} />
              </div>
